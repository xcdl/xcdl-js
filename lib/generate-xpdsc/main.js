...
var createPackage = function(cnode) {
  ...
  if (cnode.devices) {
    node.devicesFamilies = createFamilyArray(cnode.devices[0].family);
  }

  if (cnode.boards) {
    node.boards = createBoardsArray(cnode.boards[0].board);
  }
...
var createBoardsArray = function(cnode) {
...
var createBoard = function(cnode) {
  ...
  if (cnode.mountedDevice) {
    node.mountedDevices = createBoardDevicesArray(cnode.mountedDevice);
  }

  if (cnode.compatibleDevice) {
    node.compatibleDevices = createBoardDevicesArray(cnode.compatibleDevice);
  }

  return node;
};

var createBoardDevicesArray = function(cnode) {
  ...
  var len = cnode.length;
  for (i = 0; i < len; ++i) {
    var mountedDevice = cnode[i];

    nodes.push(createBoardDevice(mountedDevice));
  }
