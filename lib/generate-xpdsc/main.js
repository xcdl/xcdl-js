...
var createDevices = function(cnode) {
  ...
  for (i = 0; i < len; ++i) {
    ...
    var j;
    var families = device.family;
    var flen = families.length;
    for (j = 0; j < flen; ++j) {
      node.families.push(createFamily(families[j]));
    }
  }
  return node;
};
...
Generate_XPDSC.prototype.run = function(args, eh) {
  console.log('generate xpdsc!');
  console.log(args);

  var fs = require('fs');
  var xml2js = require('xml2js');

  var inputPath = args[2]; // Assume this returns a fully qualified XML file
  // path
  try {
    ...
    parser.parseString(inputData.substring(0, inputData.length), function(err,
      ...
      var outputData = args[4];
      fs.writeFileSync(outputData, json, 'utf8', function(err) {
